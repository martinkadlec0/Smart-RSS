...
define(['backbone', 'modules/RSSParser', 'modules/Animation', '../../libs/favicon', 'models/FeedLoader'], function (BB, RSSParser, animation, Favicon, FeedLoader) {
    ...
    let Loader = Backbone.Model.extend({
        ...
        sourcesLoading: [],
        loaders: [],
        addSources: function (source) {
            if (source instanceof Folder) {
                this.addSources(sources.where({folderID: source.id}));
                return;
            }
        ...
        download: function (sourcesToDownload) {
        ...
        workerFinished: function (worker) {
        ...
        workersFinished: function () {
            ...
            this.set('maxSources', 0);
            this.set('loaded', 0);
            this.set('loading', false);
            this.itemsDownloaded = false;
            animation.stop();
        },
        sourceLoaded: function (model) {
