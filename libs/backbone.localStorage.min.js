...
(function(root, factory) {
	...
	_.extend(Backbone.LocalStorage.prototype, {
		...
		find: function(model, cb) {
			var request = this.localStorage('readonly').get(model.id);
		...
		localStorage: (function() {
			...
			return function(type) {
				if (stores[this.name] && !type) {
					return stores[this.name];
				}

				var tx = this.db.transaction([this.name], type || 'readwrite');

				

				var tmpStore = tx.objectStore(this.name);
				if (!type && !stores[this.name]) {
					stores[this.name] = tmpStore;
					setTimeout(function() {
						stores = [];
					}, 0);
				}
